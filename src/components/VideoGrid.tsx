    ...
    <section className="px-4 sm:px-6 pb-12 sm:pb-20">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-6 sm:mb-8">
          <h3 className="text-xl sm:text-2xl font-bold text-white">[PLACEHOLDER: Section Title]</h3>
          <motion.button
          ...
          <AnimatePresence>
            {videos.map((video, index) => (
              <motion.div
                ...
                whileHover={{ y: -8 }}
                className="group relative bg-pawwelium-card rounded-2xl overflow-hidden card-hover cursor-pointer"
              >
                {/* Thumbnail - now white gradient */}
                <div className="aspect-video relative bg-gradient-to-br from-white/20 to-pawwelium-dark flex items-center justify-center text-4xl sm:text-6xl">
                  {video.thumbnail}
                  ...
                  {/* Category Badge - now white */}
                  <div className="absolute top-2 sm:top-3 left-2 sm:left-3 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full bg-black/60 backdrop-blur-sm text-xs font-medium text-white border border-white/30">
                    {video.category}
                  </div>
                </div>
